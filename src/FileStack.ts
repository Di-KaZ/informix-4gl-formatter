...
export class FileStack {
	...
	getTopStackComment(current_index: number): string {
		...
		Printer.error(`unable to find an opening statement for line ${current_index}`);
			...
			this._stack.forEach((node) => {
				Printer.error(
					`no closing statement for line ${node.line + 1} : ` +
						`'${node.content.length > 10 ? node.content.substr(0, 10) + '...' : node.content}'.`,
				);
			});
		}
	}
	...
	private static modeCondition(node: Node): string {
		return '  { ' + (node.content.length > 10 ? node.content.substr(3, 40) : node.content.slice(3)) + ' }';
	}
